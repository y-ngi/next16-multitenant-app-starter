...
- **Key Findings**:
  ...
  - `8-organization-member-management`（後続仕様）が `/dashboard/org/[orgSlug]` 配下のメンバー一覧・招待操作・ロール変更・組織削除 UI を所有すると roadmap に明記されている。したがって本仕様は `/dashboard/org/[orgSlug]` に「アクセス制御 + 組織名を表示する共通ヘッダー + 最小限のコンテキスト表示」のみを実装し、メンバー一覧/招待管理 UI の移設は行わない。

## Research Log

### slug ベースの組織解決と既存認可ロジックとの統合
- **Context**: 要件2は `/dashboard/org/[orgSlug]` の slug を組織コンテキストの唯一の情報源として扱うことを求める。既存の `requireOrganizationAccess` は `organizationId` 起点。
...
### `/dashboard/org/[orgSlug]` のレイアウトとページ責務分割
- **Context**: 要件2.4は「配下の全画面で共通ヘッダーに組織名を表示する」ことを求める。Next.js App Router で `layout.tsx` と `page.tsx` は同一リクエスト内で個別にレンダリングされる。
- **Sources Consulted**: Next.js App Router ドキュメント（layout/page の責務分離、`notFound()` の挙動）、既存の `src/app/dashboard/page.tsx`（保護ルートパターン、本仕様で `src/app/dashboard/personal/page.tsx` へ移設）
- **Findings**:
  ...
  - `notFound()` を呼ぶと、直近の `not-found.tsx`（`src/app/dashboard/org/[orgSlug]/not-found.tsx`）が使われる。
...
### 選択 UI の配置（`/dashboard/personal` と `/dashboard/personal/organizations`）
- **Context**: 要件1は両画面で所属組織の選択肢を提示し、選択時に `/dashboard/org/[orgSlug]` へ遷移させることを求める。
...
- **Findings**:
  ...
  - `/dashboard/personal/organizations` は新規ページ。`OrganizationSection`（作成ダイアログ + 一覧）をそのまま再利用すれば、`/dashboard/personal` と同一の選択体験を重複実装なしで提供できる。
- **Implications**: `OrganizationList` に「組織を開く」リンクを追加し、`/dashboard/personal/organizations/page.tsx` を新設して認証ガード後に `OrganizationSection` を描画する。
...
### Decision: `/dashboard/org/[orgSlug]` は本仕様ではアクセス制御と組織名ヘッダーのみを実装する
- **Context**: roadmap で `8-organization-member-management` が `/dashboard/org/[orgSlug]` 配下のメンバー管理 UI を所有すると明記されている。
- **Selected Approach**: `layout.tsx` で認可・共通ヘッダーを実装し、`page.tsx` は組織名・slug・自分のロールを表示する最小限のコンテキストページとする。既存のメンバー一覧/招待管理コンポーネントは `/dashboard/personal` 上に据え置き、移設しない。
- **Rationale**: 要件1〜3の範囲内に留め、8番との責務境界（roadmap 明記）を尊重する。
- **Trade-offs**: `/dashboard/org/[orgSlug]` の初期実装は情報量が少ないが、後続仕様で拡張される前提。

## Risks & Mitigations
- 既存の `OrganizationList` インライン展開と新規の `/dashboard/org/[orgSlug]` 遷移が並存し、UX が二重に見えるリスク — 本仕様では意図的に両立させ、`8-organization-member-management` でインライン展開を `/dashboard/org/[orgSlug]` 側へ統合・整理する前提とする（roadmap の依存順序どおり）。
